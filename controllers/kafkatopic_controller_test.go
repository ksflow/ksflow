...
var (
	ktNamespacedName = types.NamespacedName{Name: KTName, Namespace: testNamespace}
	kt               = &ksfv1.KafkaTopic{
...
func TestKafkaTopicReconcile(t *testing.T) {
	t.Run("create and update KafkaTopic", func(t *testing.T) {
		ctx := context.Background()
		assert.NoError(t, testK8sClient.Create(ctx, kt.DeepCopy()))
		...
		assert.NoError(t, testK8sClient.Patch(ctx, kt.DeepCopy(), crclient.RawPatch(types.MergePatchType, []byte(patchStr))))
